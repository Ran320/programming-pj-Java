...
public class GamePrinter {
    ...
    private static List<String> getBoardLines(Game currentGame) {
        ...
        if (currentGame.getMode().equals("reversi")) {
            ...
            return currentGame.board.generateGomokuBoardContent();
        }
    }
    ...
    private static List<String> getGameListLines () {
        ...
        lines.add("4.使用炸弹：gomoku模式下可使用炸弹，输入@行号+列字母以使用炸弹（eg：@2a）");
        lines.add("5.gomoku障碍：gomoku模式中#表示障碍，障碍处不可落子");
        lines.add("6.退出游戏：输入quit");
